...
export const sectionComponents: Config["components"] = {
        ...
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between gap-6 px-5">
          <Link href="/" className="flex items-center gap-3">
            ...
            <span className="leading-none">
              ...
              <span className="mt-0.5 block text-[10px] font-bold tracking-[0.15em] text-muted">{tagline}</span>
            </span>
          </Link>
          <nav className="hidden items-center gap-6 lg:flex">
            {(nav ?? []).map((n: { label: string }, i: number) => <a key={i} href="#" className="text-sm font-medium text-secondary hover:text-primary-dark">{n.label}</a>)}
          </nav>
          <div className="flex items-center gap-3">
            <Link href="/login" className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-2.5 text-sm font-semibold text-secondary hover:border-primary"><User className="h-4 w-4" /> {loginLabel}</Link>
            <Link href="/registro-mascota" className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white hover:bg-primary-dark"><PawPrint className="h-4 w-4" /> {registerLabel}</Link>
          </div>
        </div>
      </header>
    ),
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-5">
        <div className="grid grid-cols-2 gap-y-6 rounded-3xl bg-primary px-6 py-7 text-white sm:grid-cols-3 lg:grid-cols-5 lg:divide-x lg:divide-white/20">
          {(items ?? []).map((it: { label: string }, i: number) => {
            ...
            return <div key={i} className="flex items-center gap-3 px-4"><Icon className="h-7 w-7 shrink-0" /><span className="text-sm font-semibold leading-tight">{it.label}</span></div>;
          })}
        </div>
      </section>
    ),
    ...
    render: ({ leftTitle, leftHighlight, miniItems, img1, img2, rightTitle, rightHighlight, benefits, linkLabel }) => (
      <section className="py-16">
        <div className="mx-auto grid max-w-7xl gap-10 px-5 lg:grid-cols-3">
          <div>
            ...
            <div className="mt-7 grid grid-cols-4 gap-x-4 gap-y-6">
              {(miniItems ?? []).map((m: { label: string }, i: number) => {
                ...
                return <div key={i} className="text-center"><div className="mx-auto grid h-14 w-14 place-items-center rounded-full border border-border bg-white text-primary-dark shadow-sm"><Icon className="h-6 w-6" /></div><div className="mt-2 text-[11px] font-semibold text-secondary">{m.label}</div></div>;
              })}
            </div>
          </div>
          ...
          <div>
            <h2 className="text-3xl font-extrabold leading-tight text-secondary">{rightTitle} <span className="text-primary">{rightHighlight}</span></h2>
            <div className="mt-6 space-y-5">
              {(benefits ?? []).map((b: { title: string; text: string }, i: number) => {
                const Icon = benefitIcons[i % benefitIcons.length];
                return <div key={i} className="flex gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-primary-50 text-primary-dark"><Icon className="h-5 w-5" /></div><div><h3 className="font-bold text-secondary">{b.title}</h3><p className="text-sm text-muted">{b.text}</p></div></div>;
              })}
            </div>
            <a href="#" className="mt-6 inline-flex items-center gap-1 font-semibold text-primary-dark hover:underline">{linkLabel} <ArrowRight className="h-4 w-4" /></a>
          </div>
        </div>
      </section>
    ),
        ...
        <div className="relative overflow-hidden rounded-3xl bg-[#14431f] px-8 py-9 text-white">
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-4 lg:pr-40">
            {(items ?? []).map((s: { value: string; label: string }, i: number) => {
              ...
              return <div key={i} className="flex items-center gap-3"><Icon className="h-9 w-9 shrink-0 text-primary" /><div><div className="text-3xl font-black leading-none">{s.value}</div><div className="mt-1 text-xs text-white/70">{s.label}</div></div></div>;
            })}
          </div>
      ...
      <section className="py-16">
        <div className="mx-auto grid max-w-7xl gap-6 px-5 lg:grid-cols-3">
          <div className="rounded-3xl border border-border bg-white p-6 shadow-sm">
            <h3 className="font-extrabold text-secondary">{mkTitle}</h3>
            <div className="mt-5 grid grid-cols-2 gap-4">
              {(products ?? []).map((p: { name: string; price: string; image?: string }, i: number) => <div key={i}><Img src={p.image} className="h-24 w-full rounded-xl" /><div className="mt-2 text-xs font-semibold text-secondary">{p.name}</div><div className="text-xs font-bold text-primary-dark">{p.price}</div></div>)}
            </div>
          </div>
          <div className="rounded-3xl border border-border bg-white p-6 shadow-sm">
            <h3 className="font-extrabold text-secondary">{svTitle}</h3>
            <div className="mt-5 grid grid-cols-2 gap-5">
              {(services ?? []).map((s: { label: string; image?: string }, i: number) => <div key={i} className="text-center"><Img src={s.image} className="mx-auto h-20 w-20 rounded-full" /><div className="mt-2 text-xs font-semibold text-secondary">{s.label}</div></div>)}
            </div>
          </div>
          <div className="rounded-3xl border border-border bg-white p-6 shadow-sm">
            <h3 className="font-extrabold text-secondary">{bnTitle}</h3>
    ...
    render: ({ title, brands }) => (
      <section className="border-y border-border bg-surface py-10">
        <div className="mx-auto max-w-7xl px-5">
          <p className="mb-6 text-center text-sm font-semibold text-muted">{title}</p>
  ...
  TestimonialsBlock: {
    ...
    render: ({ title, items }) => (
      <section className="py-16">
        <div className="mx-auto max-w-7xl px-5">
          <div className="mb-8 flex items-center justify-between"><h2 className="text-2xl font-extrabold text-secondary">{title}</h2><a href="#" className="inline-flex items-center gap-1 text-sm font-semibold text-primary-dark">Ver más <ArrowRight className="h-4 w-4" /></a></div>
          <div className="grid gap-6 md:grid-cols-3">
            {(items ?? []).map((t: { name: string; quote: string; image?: string }, i: number) => (
              <div key={i} className="rounded-2xl border border-border bg-white p-6 shadow-sm">
                <div className="flex items-center gap-3"><Img src={t.image} className="h-12 w-12 rounded-full" /><div><div className="font-bold text-secondary">{t.name}</div><div className="flex text-amber-400">{[...Array(5)].map((_, k) => <Star key={k} className="h-3.5 w-3.5 fill-current" />)}</div></div></div>
